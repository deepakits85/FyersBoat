...
namespace FyersLoginWeb.Strategy
{
    ...
    ///   - Sequence: pehle asli first breakout, phir retracement, phir 2nd breakout → entry
    ///     (UseRetracementFirst=false). Retracement confirm = soft (pullback to EffLevel;
    ///     full 50% touch zaroori nahi). SL level ab bhi 50% structure par.
    ...
    public static class RecommendedLiveConfig
    {
        ...
        public static StrategyConfig MakeConfig(decimal riskReward)
        {
            var cfg = new StrategyConfig
            {
                ...
                // Strict sequence: first BO → retrace → 2nd BO (no retracement-before-breakout)
                UseRetracementFirst = false,
                // Ek baar pullback to EffLevel = retracement complete (full 50% touch nahi)
                SoftRetracementConfirm = true,
